...
impl<'a> Iterator for SeperateWindowsArgs<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mut quote_block_start_end = 0;
            ...
            match c {
                ...
                '"' => {
                    ...
                    if is_backslash_escaped
                        || (in_quotes && last_char == '"' && quote_block_start_end != index)
                    {
                        arg.push(c);
                    } else {
                        in_quotes = !in_quotes;
                        // This prevents that we interpret the `"` which starts
                        // or ends a quoted-block as a `"` used to escape the following `"`.
                        quote_block_start_end = index + 1;
                    }
                }
                '\\' => {
                    backslashes += 1;
